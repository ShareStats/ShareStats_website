...
## Grasple link seperately 

grasple.data <- read.csv("20250813-ShareStats-Grasple-Links.csv")[, c("item.name", "exercise.id")]
grasple_link <- "https://app.grasple.com/#/challenge/"

# Function that uses the already-loaded data
get_grasple_link <- function(name) {
  id <- grasple.data$exercise.id[match(name, grasple.data$item.name)]
  paste0(grasple_link, id)
}
...
# merge all data and grasple by common key
# NOTE! keep only the first match of Grasple duplicates @todo!
#dat.mrgd <- left_join(dat, distinct(grasple.data, name, .keep_all = TRUE), by = "name")
  ...
  mutate(Files = paste0(
    '<div class="dropdown mt-3">
      ...
      <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="">
        ...
        <li><a class="dropdown-item" href="',get_grasple_link(name),'">Grasple</a></li> 
      </ul>
    </div>'
  )) %>% 
